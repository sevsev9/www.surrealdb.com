				...
				data: [
					{
						name: 'Features',
						text: [
							'Live queries allow you to write application with realtime updates pushed to your users.',
							'Full Text Search to efficiently index and search through databases.',
							'Change feeds allow you to retrieve an ordered sequence of changes that happened to your database, enabling you to easily replicate changes.',
							'Allow segmenting custom function names with multiple <code>::</code> splits.',
							'Types in SurrealQL are now strict and can be guaranteed.',
							...
							'Introduce new <code>string::contains</code> string function.',
							'Introduce new <code>time::ceil</code>, <code>time::min</code>and <code>time::max</code> time functions.',
							...
							'Added a CLI <code>--json</code> output option to return JSON instead of SurrealQL objects.',
							'Use full words instead of abbreviations for the <code>INFO</code> command.',
							'Added a SpeeDB storage engine implementation.',
							'Ability to change behaviour for detecting client IP-addresses.',
							'Add authentication support for local engines',
							'Added configurable global query timeouts.',
							'Added the full <code>fetch</code>-suite into our embedded scripting functions.',
							'Added a <code>query</code> function into our embedded scripting functions, enabling the execution of surrealql within them.',
							'Added a <code>value</code> function into our embedded scripting functions, enabling the retrieval of variables from the current execution context.',
							'Exposed all built-in SurrealQL functions to the embedded scripting functions.',
							'Ability to check version for remote instances using the CLI.',
							'Added a <code>surreal validate [file]</code> command to validate SurrealQL queries.',
							'Added additional functions for comparing strings for similarity.',
							'Ability to reference an object key by a string, variable or document property.',
							'Implemented the <code>insert</code> method in the websocket protocol to allow for bulk insertion of records.',
							'Ability to create multiple root-level users.',
							'Introduction of basic RBAC functionality.',
							'Enable or disable and allow or restrict certain capabilities of SurrealDB.',
							'Implemented missing JSON Patch operations (move, copy and test).',
							'Introducing a new <code>DEFAULT</code> clause on field definitions.',
							'Introducing a simplified <code>IF-ELSE</code> syntax when using a Block as the body.',
							'Introducing the new <code>FOR</code>, <code>CONTINUE</code>, <code>BREAK</code> and <code>THROW</code> statements.',
							'Added a <code>COMMENT</code> clause on all resource definitions, allowing to add a description to such.',
							...
							'Ability to <code>OMIT</code> certain fields from a selection.',
							'Introduce <code>type::field()</code> and <code>type::fields()</code> methods to dynamically select properties from a resource.',
							'Ability to set permissions on <code>PARAM</code>s and custom <code>FUNCTION</code>s.',
						],
					},
					{
						name: 'Bug fixes',
						text: [
							...
							{
								category: 'bug',
								text: 'If any query in a set of queries were to fail in the CLI, only the error would be shown. This is now fixed by including the error in an array of results.',
							},
							{
								category: 'bug',
